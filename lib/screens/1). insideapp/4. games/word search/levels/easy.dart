...
class WordSearchEasy extends StatefulWidget {
  const WordSearchEasy({Key? key}) : super(key: key);

  @override
  _WordSearchEasyState createState() => _WordSearchEasyState();
}

class _WordSearchEasyState extends State<WordSearchEasy> {
  final int numBoxPerRow = 4;
...
//COPY DITO
  ...
  void onDragEnd([PointerUpEvent? event]) {
    ...
    if (indexFound >= 0) {
      ...
      if (answerList.value.every((answer) => answer.done)) {
        // Show the dialog if all words are found
        loadFinishModal();
      }
    }
  ...
  void generateRandomWord() {
    final List<String> wl = ['AT', 'ON', 'IN', 'DOG', 'CAT', 'BAG'];

    // Shuffle and select 3 random words from the list
    wl.shuffle();
    final List<String> selectedWords = wl.take(3).toList();
    ...
    final WSNewPuzzle newPuzzle = wordSearch.newPuzzle(selectedWords, ws);

    if (newPuzzle.errors!.isEmpty) {
      listChars.value = newPuzzle.puzzle ?? [];

      final WSSolved solved =
          wordSearch.solvePuzzle(newPuzzle.puzzle ?? [], selectedWords);
